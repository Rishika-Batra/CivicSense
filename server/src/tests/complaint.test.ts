...
const citizenToken = jwt.sign(
  { id: CITIZEN_ID, role: 'citizen', email: 'citizen@test.com' },
  SECRET
)
const officerToken = jwt.sign(
  { id: OFFICER_ID, role: 'officer', email: 'officer@test.com' },
  SECRET
)
...
describe('POST /api/complaints', () => {
  ...
  it('returns 201 when a citizen files a valid complaint', async () => {
    jest.spyOn(Complaint, 'create').mockResolvedValue({
...
describe('PATCH /api/complaints/:id/status', () => {
  ...
  it('returns 200 when the assigned officer updates complaint status', async () => {
    ...
    const officerObjectId = new Types.ObjectId(OFFICER_ID)
    ...
    jest.spyOn(Complaint, 'findById').mockResolvedValue(mockComplaint as any)
